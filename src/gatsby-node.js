...
exports.sourceNodes = async (
  { actions, getNode, getNodes, createNodeId, hasNodeChanged, store, webhookBody },
  { authToken, contentFields, pageTypes, locales, preview }
) => {
  ...
  //Support Webhook events to deleteNode and createNode
  ...
  async function create_nodes_collections(locale) {
    ...
    if (contentFields) {
      ...
      if (contentFieldsResult.data.data) {
        ...
        allCollection.forEach(({ key, value }) => {
          if (locale) {
