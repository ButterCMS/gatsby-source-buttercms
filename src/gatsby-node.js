...
exports.sourceNodes = async ({
  ...
  const {
    ...
    touchNode,
    setPluginStatus
  } = actions; // Authenticate ButterCMS API client.

  const api = butter(authToken, false, 20000); // Touch existing ButterCMS nodes so Gatsby doesn’t garbage collect them.

  Object.values(store.getState().nodes).filter(n => n.internal.type.slice(0, 8) === typePrefix).forEach(n => touchNode(n.id)); // Paginate through all blog posts
  ...
  async function create_nodes_collections(locale) {
    ...
    if (contentFields) {
      ...
      if (contentFieldsResult.data.data) {
        ...
        let allCollection = [],
            allfields = []; //Extract Collections and Content Fields into separate variables
        ...
        allCollection.forEach(({
          ...
          const collectionNode = Object.assign({}, {
            ...
            internal: {
              ...
              contentDigest: crypto.createHash(`md5`).update(JSON.stringify([key, value])).digest(`hex`)
            },
            ...params
          });
